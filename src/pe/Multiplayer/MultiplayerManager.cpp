...
HkTrampoline<void, SingleModeScene*> singleModeSceneControlHook = hk::hook::trampoline([](SingleModeScene* scene) {
    {
    ...
    singleModeSceneControlHook.orig(scene);
});

HkTrampoline<void, SingleModeScene*, const al::SceneInitInfo&> singleModeSceneInitHook = hk::hook::trampoline([](SingleModeScene* scene, const al::SceneInitInfo& info) {
    singleModeSceneInitHook.orig(scene, info);
...
HkTrampoline<void, al::Scene*> sceneDestroyHook = hk::hook::trampoline([](al::Scene* thisPtr) {
    MultiplayerManager::instance()->deinitPuppets();
    sceneDestroyHook.orig(thisPtr);
});
...
HkTrampoline<void, PlayerAnimator*, const sead::SafeString&> startAnimHook = hk::hook::trampoline([](PlayerAnimator* thisPtr, const sead::SafeString& anim) {
    if (sStartAnimHookEnabled)
        sendAnimPacket(thisPtr, anim, enet::ActionPacketData::Type::Anim);

    startAnimHook.orig(thisPtr, anim);
});

HkTrampoline<void, PlayerAnimator*, const sead::SafeString&> startSubAnimHook = hk::hook::trampoline([](PlayerAnimator* thisPtr, const sead::SafeString& subAnim) {
    if (sStartAnimHookEnabled)
        sendAnimPacket(thisPtr, subAnim, enet::ActionPacketData::Type::SubAnim);

    startSubAnimHook.orig(thisPtr, subAnim);
});

HkTrampoline<void, PlayerAnimator*, const sead::SafeString&> startMaterialAnimHook = hk::hook::trampoline([](PlayerAnimator* thisPtr, const sead::SafeString& matAnim) {
    if (sStartAnimHookEnabled)
        sendAnimPacket(thisPtr, matAnim, enet::ActionPacketData::Type::MaterialAnim);

    startMaterialAnimHook.orig(thisPtr, matAnim);
});
...
void MultiplayerManager::init()
{
    ...
    startAnimHook.installAtOffset(hk::ro::getMainModule(), 0x003747a0);
    startSubAnimHook.installAtOffset(hk::ro::getMainModule(), 0x00374c60);
    startMaterialAnimHook.installAtOffset(hk::ro::getMainModule(), 0x00375360);
    hk::hook::writeBranchLink(hk::ro::getMainModule(), 0x00146958, nameplateHideModelFix);
    singleModeSceneControlHook.installAtOffset(hk::ro::getMainModule(), 0x003e84d0);
    singleModeSceneInitHook.installAtOffset(hk::ro::getMainModule(), 0x003e7090);
    sceneDestroyHook.installAtOffset(hk::ro::getMainModule(), 0x008a40e0);
    hk::hook::writeBranchLink(hk::ro::getMainModule(), 0x003601ac, playerActorInitHook);
    initGoalItemSyncHooks();
    puppets::initPlayerBonkHook();
    installMiscMapSyncHooks();
    hk::hook::writeBranchLink(hk::ro::getMainModule(), 0x003e7c28, layoutInitHook);
}
