...
HkTrampoline<int, al::SaveDataSequenceRead*, const char*> saveDataSequenceReadHook = hk::hook::trampoline([](al::SaveDataSequenceRead* thisPtr, const char* path) {
    if (getSaveDataMode() == SaveDataMode::Local)
        return saveDataSequenceReadHook.orig(thisPtr, path);
    else {
...
HkTrampoline<int, al::SaveDataSequenceWrite*, const char*> saveDataSequenceWriteHook = hk::hook::trampoline([](al::SaveDataSequenceWrite* thisPtr, const char* path) {
    if (getSaveDataMode() == SaveDataMode::Local)
        return saveDataSequenceWriteHook.orig(thisPtr, path);
    else {
        if (thisPtr->mSaveBufferSize == 0)
            return 0;
...
{
    hk::hook::writeBranchLink(hk::ro::getMainModule(), 0x0041b2a4, requestWriteSaveDataHook);
    hk::hook::writeBranchLink(hk::ro::getMainModule(), 0x0041b198, requestReadSaveDataHook);
    hk::hook::writeBranchLink(hk::ro::getMainModule(), 0x0041b47c, requestInitSaveDataHook);
    saveDataSequenceReadHook.installAtOffset(hk::ro::getMainModule(), 0x008a2fa0);
    saveDataSequenceWriteHook.installAtOffset(hk::ro::getMainModule(), 0x008a3210);
}
