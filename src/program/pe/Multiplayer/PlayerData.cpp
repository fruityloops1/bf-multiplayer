    ...
    : mListData(packetData)
{
    ...
    Patcher(0x003bd0a0).BranchLinkInst((void*)raidonHook<true>);
    Patcher(0x003bd494).BranchLinkInst((void*)raidonHook<false>);
}
...
void PlayerData::updateLocal(MultiplayerManager* mgr)
{
    ...
    if (mIsHostClient && (mFrameCount % 120 == 0 or isNewNerve)) {
        ...
        data.controllerNerve = uintptr_t(curNerve) - exl::util::modules::GetTargetStart();
