...
#define write8(byte) (*(outat++) = byte)
        ...
        while (patchat < patchend) {
            ...
            case TargetCopy: {
                ...
                if (outreadat < outstart || outreadat >= outat || outreadat + length > outend)
                    error(bps_broken);
                for (size_t i = 0; i < length; i++)
                    write8(*outreadat++);
            } break;
...
#define write(val)                                                   \
    do {                                                             \
        ...
        if (outlen == outbuflen) {                                   \
            outbuflen *= 2;                                          \
            uint8_t* newout = (uint8_t*)reallocImpl(out, outbuflen); \
            if (!newout) {                                           \
                freeImpl(out);                                       \
...
enum bpserror bps_create_linear(struct mem sourcemem, struct mem targetmem, struct mem metadata, struct mem* patchmem)
{
    if (sourcemem.len >= (SIZE_MAX >> 2) - 16)
        return bps_too_big; // the 16 is just to be on the safe side, I don't think it's needed.
    ...
    uint8_t* out = (uint8_t*)mallocImpl(outbuflen);
    if (!out)
        return bps_out_of_mem;
    size_t outlen = 0;
    write('B');
    ...
    while (target < targetend) {
        ...
        while (source + numunchanged < sourceend && source[numunchanged] == target[numunchanged])
            numunchanged++;
        ...
        while ((source + numchanged >= sourceend || source[numchanged] != target[numchanged] || source[numchanged + 1] != target[numchanged + 1] || source[numchanged + 2] != target[numchanged + 2]) && target + numchanged < targetend) {
            numchanged++;
        ...
        if (numchanged) {
            ...
            while (true) {
                if (
                    target[rle1start - 1] == target[rle1start + 0] && target[rle1start + 0] == target[rle1start + 1] && target[rle1start + 1] == target[rle1start + 2] && target[rle1start + 2] == target[rle1start + 3]) {
                    numchanged = rle1start;
                    break;
                }
                if (
                    target[rle1start - 2] == target[rle1start + 0] && target[rle1start - 1] == target[rle1start + 1] && target[rle1start + 0] == target[rle1start + 2] && target[rle1start + 1] == target[rle1start + 3] && target[rle1start + 2] == target[rle1start + 4]) {
                    numchanged = rle1start;
                    break;
                }
            ...
            if (target[-2] == target[0] && target[-1] == target[1] && target[0] == target[2]) {
                // two-byte RLE
                size_t rlelen = 0;
                while (target + rlelen < targetend && target[0] == target[rlelen + 0] && target[1] == target[rlelen + 1])
                    rlelen += 2;
...
#define error(why)       \
    ...
    if (changefrac && ret.size_in > 0) {
        ...
        uint8_t* patchbin = (uint8_t*)mallocImpl(len);
        ...
        for (int i = 0; i < 2; i++) {
            if (step[i]) {
                ...
                if (action == SourceCopy || action == TargetCopy) {
                    ...
                    sprintf(describe[i], "%s from %zu for %zu in %zu", actionnames[action], patchoutpos[i], len, patchpos[i] - patchposstart);
                if (!step[i ^ 1]) {
                    printf("%i: %s\n", i + 1, describe[i]);
                    show = true;
                }
...
static struct mem ReadWholeFile(const char* filename)
{
    ...
    unsigned char* data = (unsigned char*)mallocImpl(len);
